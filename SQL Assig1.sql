...
create table 
desig(desigcode varchar(15), designame varchar(15)); 
...
select empname,deptcode,birthdate
from emp where empcode='6569';
...
where deptcode in
(select deptcode from dept 
where deptname='ACCOUNTS' OR deptname='Purchase'
);
...
from emp where 
birthdate between '1982-12-31' and '1992-12-31';


select empname from emp 
where (empname) like 'Si%';

select empname,supcode 
from emp where supcode is null;
select (current_date - birthdate) from emp

select empname,birthdate, joindate, sex from emp where sex='F'
where (birthdate<(current_date-50*365) 
or joindate<(current_date-20*365) 
and sex='F';

select current_date;


select (basic+allow- deduct)*.01 
as TakehomePay from salary
...
from emp where basicpay >= 
all(select basicpay from emp);

select max(basicpay),empname
from emp 

select empname,basicpay 
from emp order by(basicpay);
...
-- Queries are started from below Q1 to Q48 ----------------------------
...
-- Q3 -----------
Select e.empname As EmployeeName, d.deptname As DepartmentName From emp e
